...
For example if viewport has \texttt{clip} set to ``on", then any viewport created within that viewport which uses ``inherit" will clip to the same region as the parent viewport.
