...
This is because \grid{}'s graphic parameters defines a line's height to be $\mathtt{fontsize} \times \mathtt{cex} \times \mathtt{lineheight}$.
...
After implementing solutions the two key problems relating to multi-line text we are able to accurately draw \grid{} text graphics objects.
...
Because the type of the font is unknown, we cannot provide reasonable fallback fonts.
We therefore assume the font is sans-serif and will therefore show a reasonable sans-serif font in the event that the user-specified font is not present.
...
The solution to the \texttt{vp} problem will not be discussed due to the advanced knowledge of \grid{} required to understand it.
...
There is in fact one \texttt{frame} that contains several \texttt{cellGrob}s, that contains the graphics objects that are components of the legend.
...
The last area in which \gridSVG{} was deficient in its support of points was that the size of its points were often incorrect.

To fix the first problem, the task of observing plotting characters present in \R{} and translating them to \grid{} graphical objects occurred.
...
Now that we can draw many plotting characters, it would be useful to be able to more than one when plotting a set of points.
The correction to \gridSVG{} required vectorising the code so that checked which plotting character to use for each point.
Previously it would only check for the plotting character that would be used on the first point and apply that to every point.
...
We can see that although the text ``Hello, world!" is drawn large enough to exceed the dimensions of the viewport, it is partially obscured.
...
For example if viewport has \texttt{clip} set to ``on", then any viewport created within viewport that uses ``inherit" will clip to the same region as the parent viewport.
...
This does not require much extra work because \gridSVG{} already created a \texttt{<g>} element to group all of the graphics objects together that are drawn within it.
...
When a viewport was applied more than once, we ended up with multiple \texttt{<g>} elements with the same name.
In doing so, when we would also create multiple clipping paths, all with the same name.
...
This is because we only get to observe one viewport at a time.
...
While most of the work involved in the development of \gridSVG{} involved creating accurate graphical output, a feature of \gridSVG{} that was also improved was it's ability to animate graphical objects.
...
The \texttt{<animate>} element requires three key pieces of information: the name of an element, the attribute to be animated, values that the attribute is going to animate through.
...
The most important of the changes is that there is no longer the assumption that one graphics object produces one SVG element.
...
The columns of each matrix refer to each of the sub graphics objects that a graphics object produces.
...
If a rectangle is one wide, anything that animates the rectangle's width must be measured in inches.

The reason for these restrictions is the fact that matrices can be composed of either numeric, logical or character values.
There is therefore no way of using \grid{}'s unit type to specify values to animate through.
Any parameters of graphics objects that are character or logical values have no obvious drawbacks when using matrices to animate their values.

When improving \texttt{grid.animate()} another issue that had to be mitigated was the fact that points can be composed of several types of graphics objects.
Consequently, a parameter of a \grid{} graphics object can map to many different SVG attributes.
An example of this a plotting character composed of a circle and a couple of lines (see \autoref{subfig:pch10}).
...
Several graphics objects can now be animated along many of their properties.
